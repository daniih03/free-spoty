...
16. **Importar playlists de Spotify por link (`ImportExportModal.tsx`, `services/spotifyImportService.ts`):**
   ...
   - El widget de Spotify no envía cabeceras CORS: si el servidor propio (`server/`) está activo se le pide a él (`GET /api/spotify-playlist?id=`, sin problema de CORS); si no, se usa como respaldo el proxy público `api.allorigins.win`, menos fiable — si falla, se muestra un aviso y se puede reintentar.
