...
16. **Importar playlists de Spotify por link (`ImportExportModal.tsx`, `services/spotifyImportService.ts`):**
   - Pegar el link de una playlist pública de Spotify (`open.spotify.com/playlist/<id>`) basta: se extrae el ID, se lee el JSON embebido del widget público de Spotify (`__NEXT_DATA__` en `open.spotify.com/embed/playlist/<id>`, sin necesidad de credenciales de API) y se obtiene nombre, carátula y tracklist completo.
