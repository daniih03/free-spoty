...
15. **Rediseño visual v3 "Laca hi-fi" (sept. 2026):**
   ...
   - **Vistas:** Inicio con saludo grande + accesos rápidos + fundas de playlists + estanterías horizontales con snap; Buscar con géneros tipográficos y "Mejor resultado + Canciones"; Playlist y Artista con cabecera a sangre teñida con el color de la carátula y tracklist numerado; letras en tipografía grande con la línea activa iluminada sobre la carátula difuminada. La barra superior flota sobre el contenido y gana fondo al desplazarse.

16. **Importar playlists de Spotify por link (`ImportExportModal.tsx`, `services/spotifyImportService.ts`):**
   - Pegar el link de una playlist pública de Spotify (`open.spotify.com/playlist/<id>`) basta: se extrae el ID y se piden nombre, carátula y tracklist al servidor propio.
   - Cada pista se empareja luego con `searchSongsMetadata` (iTunes) igual que la importación manual por texto, con 4 búsquedas en paralelo y barra de progreso.
   - Si el servidor propio (`server/`) está activo se le pide a él (`GET /api/spotify-playlist?id=`, sin problema de CORS); si no, se usa como respaldo el lector `r.jina.ai` sobre el widget público (refleja `Access-Control-Allow-Origin`, cabecera `X-Return-Format: html` para conservar el `__NEXT_DATA__`) — si falla, se muestra un aviso y se puede reintentar. (`api.allorigins.win`, usado antes, quedó caído de forma permanente — 522 — en sept. 2026.)
   - **Límite de ~100 pistas con el widget:** el JSON embebido del widget público (`__NEXT_DATA__` en `open.spotify.com/embed/playlist/<id>`) recorta la playlist a ~100 pistas y no pagina (`?offset=` no tiene efecto) — playlists más largas se importan incompletas. El servidor propio evita esto pagina la Web API oficial (`/v1/playlists/{id}/tracks`) usando, en orden: Client Credentials (`SPOTIFY_CLIENT_ID`/`SPOTIFY_CLIENT_SECRET`, requiere Premium en la cuenta dueña de la app desde 2026) o, gratis y sin cuenta, el token anónimo del propio widget; si ambos fallan cae al widget capado. Ver `server/README.md`.
   - Sigue admitiendo pegar una lista manual de "Artista - Canción" como hasta ahora; ambos modos comparten el mismo formulario y pipeline de emparejado.
