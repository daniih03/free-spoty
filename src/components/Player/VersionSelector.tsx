...
import { Radio, FileText, Film, Sparkles } from 'lucide-react';
...
export const VersionSelector: React.FC<VersionSelectorProps> = ({ compact = false }) => {
  ...
  const versions: { type: VersionType; label: string; shortLabel: string; icon: React.ReactNode; tooltip: string }[] = [
    {
      ...
      tooltip: 'Versión limpia sin intros largas de videoclip ni silencios',
    },
    {
      type: 'lyrics',
      label: 'Lyrics',
    ...
    },
    {
      ...
      tooltip: 'Versión oficial original del tema',
    },
  ];
  ...
  return (
    <div
      ...
      title="Prioridad de audio inteligente: Radio Edit > Lyrics > Original"
    >
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium transition-all duration-200 ${
              isActive
                ? 'bg-gradient-to-r from-brand-green to-emerald-500 text-black font-semibold shadow-lg shadow-emerald-500/20 scale-105'
