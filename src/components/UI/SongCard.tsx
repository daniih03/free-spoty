  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square w-full rounded-xl md:rounded-[16px] overflow-hidden mb-3 shadow-md bg-white/[0.03]">
        ...
        <button
          onClick={handleLike}
          className={`absolute top-2 left-2 p-2 rounded-full backdrop-blur-md transition-all z-10 touch-manipulation active:scale-90 ${
            isLiked
              ? 'bg-black/50 text-brand-coral opacity-100 shadow-sm'
              : 'bg-black/40 text-white/80 hover:text-white hover:bg-black/60 opacity-80 md:opacity-0 md:group-hover:opacity-100'
          }`}
        ...
        >
          <button
            ...
            className="w-10 h-10 rounded-full bg-gradient-to-tr from-brand-crimson via-brand-red to-brand-coral text-white flex items-center justify-center shadow-lg shadow-brand-red/40 hover:scale-110 active:scale-95 transition-all touch-manipulation"
      ...
      <div className="flex items-start justify-between gap-1.5 px-0.5">
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          <p
            ...
            className={`text-xs text-white/45 truncate transition-colors ${
              onNavigateArtist
                ? 'hover:underline hover:text-brand-rose cursor-pointer active:opacity-75'
                : 'hover:text-white/70'
            }`}
          >
            {song.artist}
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="p-2 rounded-lg text-white/60 hover:text-white hover:bg-white/10 opacity-90 md:opacity-0 md:group-hover:opacity-100 transition-all touch-manipulation active:scale-95"
            title="Más opciones"
          >
              ...
              >
                {onNavigateArtist && (
                  <button
                    ...
                    className="w-full text-left px-3.5 py-2.5 hover:bg-white/10 flex items-center gap-2.5 text-white/90 transition-colors touch-manipulation"
                  >
